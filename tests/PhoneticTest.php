...
class PhoneticTest extends TestCase
{
    ...
    {
        $this->assertSame($expected, Phonetic::encode($input), "$input => $expected");
    }
    ...
    {
        $this->assertSame('', Phonetic::encode(''));
    }

    public function test_homophones(): void
    {
        // Sceau, Seau, Sot, Saut should all produce 'SO'
        $codes = array_map(fn ($w) => Phonetic::encode($w), ['Sceau', 'Seau', 'Sot', 'Saut']);
        $this->assertCount(1, array_unique($codes));
        $this->assertSame('SO', current($codes));

        // Verre, Vert, Vers should all produce 'VER'
        $codes = array_map(fn ($w) => Phonetic::encode($w), ['Verre', 'Vert', 'Vers']);
        $this->assertCount(1, array_unique($codes));
        $this->assertSame('VER', current($codes));

        // Saule, Sol should produce 'SOL'
        $codes = array_map(fn ($w) => Phonetic::encode($w), ['Saule', 'Sol']);
        $this->assertCount(1, array_unique($codes));
        $this->assertSame('SOL', current($codes));
    }
}
